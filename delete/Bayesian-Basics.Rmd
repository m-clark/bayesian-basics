...
```{r maxLikeillustration, cache=FALSE, eval=T, echo=FALSE, out.width='500px', message=T}
set.seed(1234)
...
```{r plotSurface, eval=T, echo=FALSE}
...
plot_ly(x=~mu, y =~ sigma, z=~llsurf_trans, 
        colors = viridis::plasma(500), 
        type='surface', showscale = FALSE, width=750) %>% 
  add_trace(x=~mu, y=~sigma, z=~ll, mode='markers', marker=list(color=palettes$orange, size=3), type = "scatter3d", 
            data=pointdat, name=NULL, showlegend=F) %>%
  add_trace(x=coef(mlnorm)[1], y=coef(mlnorm)[2],  z=mlnorm@min, 
            marker=list(color=palettes$orange$complementary, size=10), type = "scatter3d", showlegend=F) %>%
  config(displayModeBar = F) %>%
  theme_plotly() %>% 
  layout(scene = list(
    xaxis = list(title = 'mu',
...
```{r plotNormMLEpath, eval=T, echo=FALSE, out.width='500px'}
# plotdat = read.csv('data//mleNormEstimates.csv'); colnames(plotdat) = c('mu','sigma', 'll')
...
```{r mlmisc, eval=T, echo=FALSE}
detach(package:bbmle) # why are people still using MASS?!
plotdat = read.csv('data//mleEstimates.csv'); colnames(plotdat) = c('sigma', 'Intercept', 'β', 'll')
...
plotdat %>% 
  ...
  ggplot() + 
  geom_point(aes(x=sigma, y=value, size=nll), color=palettes$orange$orange, 
             alpha=.15, show.legend=F, position = position_jitter(w = 0.2, h = 0.2)) +
