...
In the following code, we note the object that contains the Stan model code, the data list, how many iterations we want (5000)[^toomanyiter], how long we want the process to run before we start to keep any estimates (`warmup=2500`), how many of the post-warmup draws of the posterior we want to keep (`thin=10` means every tenth draw), and the number of chains (`chains=4`).  In the end we will have three chains of 1000[^finaliter] draws from the posterior distribution of the parameters.  Stan spits out a lot of output to the R console even with `verbose = FALSE`, and I omit it here, but you will see some initial info about the compiling process, updates as each chain gets through 10% of iterations specified in the `iter` argument, and finally an estimate of the elapsed time.  You may also see *informational messages* which, unless they are highly repetitive, should not be taken as an error.
...
fit = stan(model_code=stanmodelcode, data=dat, iter=5000, 
           warmup=2500, thin=10, chains=4)
```

```{r rstanModelsave, echo=F, eval=F}
...
[^finaliter]: $\frac{5000-2500}{10} \cdot 4 = 1000$
...
```{r posteriorPredictiveExample, fig.show='hide', echo=6:14}
load('data/mainModels.RData')
library(rstan)
...
```{r posteriorPredictiveMinComparison, fig.keep='none', fig.show='hide', echo=1:7}
...
```{r posteriorPredictiveIndividualy, echo=FALSE, out.width='500px'}
...
```{r maxLikeillustration, cache=FALSE, echo=FALSE, out.width='500px', message=T}
set.seed(1234)
...
```{r bugsRunModel, eval=FALSE, echo=-c(1:5, 9)}
#####################
### Run the model ###
#####################
...
round(lmbugs$summary[,c(1:3,5,7:9)], 3) %>% pander
lmbugs$summary
```

The usual model diagnostics are available with conversion of the results to an object the <span class="pack">coda</span> package can work with. Figures are not shown, but they are the typical traceplots and density plots.
...
```{r jagsModelResults_pretty, echo=F}
load('data/jagsModel.RData')
broom::tidyMCMC(lmjags, conf.int=T) %>% format(digits=2) %>% pander(justify='lrrrr')
coda::effectiveSize(lmjags) %>% pander
```
...
# get final chain
library(coda)
bfinal = as.mcmc(data.frame(beta=b[seq(burnin+1, nsim, by=thin),], sigmasq=s2[seq(burnin+1, nsim, by=thin)]))

# get summaries; compare to lm and stan
# broom::tidyMCMC(bfinal, conf.int=T) %>% format(digits=2) %>% pander(justify='lrrrr')
rstan::monitor(array(cbind(b,s2),dim=c(5000,1,5), dimnames=list(NULL, NULL, colnames(bfinal))), 
               warmup=burnin, digits_summary=5, probs=c(.025,.975), print=F) %>% 
  format(digit=2) %>% 
  pander(justify='lrrrrrrr')
# round(c(coef(modlm), sigamsq=summary(modlm)$sigma^2), 3) %>% pander
```
...
broom::tidy(fit, conf.int=T) %>%  format(digits=2) %>% pander(justify='lrrrr')
```
...
# b = finalest[1:4]
# sig = finalest[5]
# broom::tidyMCMC(theta, conf.int=T) %>% format(digits=2) %>% pander(justify='lrrrr')
rstan::monitor(M1$sims, 
               warmup=500, digits_summary=5, probs=c(.025,.975),
               print=F) %>% 
  round(digit=3) %>%
  format(digit=3) %>%
  pander(justify='lrrrrrrr')
...
broom::tidy(fitIG, conf.int=T) %>% format(digits=3) %>% pander(justify='lrrrr')
...
The following are three texts I recommend in my consulting to folks interested in doing Bayesian data analyis. They can be seen as introduction, moderate, and advanced respectively
