...
The overall process allows it to move quite rapidly through the parameter space, and it can work well where other approaches such as Gibbs might be very slow.  An example using HMC on the regression model data can be found in the [Hamiltonian Monte Carlo Example][Hamiltonian Monte Carlo Example]<span class="marginnote">See this entry at David Mimno's blog for a [visualization of HMC](http://www.mimno.org/articles/hmc/).</span>
